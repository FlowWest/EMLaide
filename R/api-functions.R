...
#' @examples
#' \dontrun{
#' reserve_edi_id(api_key = Sys.getenv("EDI_API_KEY"))}
#' @export

reserve_edi_id <- function(api_key, environment = c("production", "staging", "development")) {
  environment <- match.arg(environment)

  base_url <- as.character(BASE_URLS[environment])

  response <- httr::POST(
    url = httr::modify_url(base_url, path = "package/reservations/eml/edi", query = list(key = api_key))
  )
...
#' @examples
#' \dontrun{evaluate_edi_package(api_key = Sys.getenv("EDI_API_KEY"),
#'                               eml_file_path = "data/edi20.1.xml")}
#' @export
evaluate_edi_package <- function(api_key, eml_file_path,
  ...
  response <- httr::POST(
    url = httr::modify_url(base_url, path = "package/evaluate/eml", query = list(key = api_key)),
...
#' @examples
#' \dontrun{upload_edi_package(api_key = Sys.getenv("EDI_API_KEY"),
#'                             eml_file_path = "data/edi20.1.xml")}
#' @export
upload_edi_package <- function(api_key, eml_file_path, environment = "production",
                               .max_timeout = 5) {
  ...
  response <- httr::POST(
    url = httr::modify_url(base_url, path = "package/eml/", query = list(key = api_key)),
    body = httr::upload_file(eml_file_path)
  )
  ...
  if (identical(response$status_code, 202L)) {
    ...
    check_error <- httr::GET(
      url = httr::modify_url(base_url, path = glue::glue("package/error/eml/{transaction_id}"), query = list(key = api_key))
    )
...
#' @return Message describing if your package was successfully updated or not.
#' @examples
#' \dontrun{update_edi_package(api_key = Sys.getenv("EDI_API_KEY"),
#'                             existing_package_identifier = "edi.740.1",
#'                             eml_file_path = "data/edi20.1.xml")}
#' @export
update_edi_package <- function(api_key, existing_package_identifier,
  ...
  response <- httr::PUT(
    url = httr::modify_url(base_url, path = glue::glue("package/eml/{scope}/{identifier}"), query = list(key = api_key)),
    body = httr::upload_file(eml_file_path)
  )
  ...
  if (identical(response$status_code, 202L)) {
    ...
    check_error <- httr::GET(
      url = httr::modify_url(base_url, path = paste0("error/eml/", transaction_id), query = list(key = api_key))
    )
...
#' @param api_key EDI API access key
