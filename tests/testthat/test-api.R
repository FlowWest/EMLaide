...
test_that("reserve_edi_id() errors when arguments are not supplied", {
  expect_error(reserve_edi_id(), "argument \"api_key\" is missing, with no default")
})

test_that("reserve_edi_id() returns an edi number", {
  vcr::use_cassette("reserve_edi_id_responses", {
    id <- reserve_edi_id(Sys.getenv("EDI_API_KEY"))
  })
...
test_that("evaluate_edi_package() errors when arguments are not supplied", {
  expect_error(evaluate_edi_package(api_key = Sys.getenv("EDI_API_KEY")), "argument \"eml_file_path\" is missing, with no default")
  expect_error(evaluate_edi_package())
})
...
test_that("evaluate_edi_package() returns the anticipated response object", {
  ...
  expect_equal(evaluate_edi_package(api_key = Sys.getenv("EDI_API_KEY"),
